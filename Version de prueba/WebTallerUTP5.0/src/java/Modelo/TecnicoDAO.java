...
int r;
   ...
   ArrayList<Tecnico>list=new ArrayList<>();
       String sql="select * from persona AS p INNER JOIN horarios AS h ON p.id_horario=h.id_horario where p.id_tipo_persona = 1 and p.estado_persona = 1";
        try {
            ...
            while(rs.next()){
                ...
                tec.setNombre_horario(rs.getString("nombre_horario"));
                tec.setDescripcion_horario(rs.getString("descripcion_horario"));
    ...
    public Persona list(int dni) {
       String sql="select * from persona AS p INNER JOIN horarios AS h ON p.id_horario=h.id_horario where dni="+dni;
        try {
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery();
            while(rs.next()){
                ...
                t.setHorario(rs.getInt("id_horario"));
                t.setNombre_horario(rs.getString("nombre_horario"));
                t.setDescripcion_horario(rs.getString("descripcion_horario"));
    ...
    public int edit(Tecnico tec) {
        ...
        String sql = "update persona set dni='" + tec.getDNI() + "', "
                + "nombres='" + tec.getNombre() + "',"
                + " apellido_paterno='" + tec.getAp_paterno() + "', "
                + "apellido_materno='" + tec.getAp_materno() + "', "
                + "id_tipo_persona='1',"
                + "id_distrito=null,"
                + "id_horario='" + tec.getHorario() + "', "
                + "direccion=null,"
                + "telefono='" + tec.getTelefono() + "',"
                + "correo_electronico='" + tec.getCorreo_electronico() + "',"
                + "especialidad='" + tec.getEspecialidad() + "',"
                + " experiencia_laboral='" + tec.getExperiencia_labo() + "' "
                + "where dni=?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, tec.getDNI());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace(System.out);
        }
